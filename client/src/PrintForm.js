...
const PrintForm = ({ printFormArgs }) => {
    const { setFormData, pullFormData, formData, truncateString, handlename, name, supervisorPrint, email, handleemail,
        ...
        cureTime, handleCureTime, filamentSettings, jobMaterial, handleJobMaterial, isSpecialty, materialAllowed } = printFormArgs
        ...
        <>
            ...
            <div className='printForm'>
                ...
                <div style={{ marginTop: '15px' }}> -- Notes (Optional) --
                    <br />
                    <textarea value={notes} type="text" onChange={handlenotes} className={"job-notes"}></textarea>
                </div>
                {isSpecialty(selectedPrinter) && <>
                    <br />
                    <div style={{ height: '10px' }}></div>
